...
public:
	...
	std::string toString() {
		char s[6000];
		sprintf_s(s, 6000, "%s, Space Number: %d, Size: %s, Weight: %s, Insurance Cost: %d", order.toStringWSS().c_str(), spaceNumber, size.c_str(), weight.c_str(), insuranceCost); // companyName, order.wagonNumber, order.shippingDate, order.shippingCost, spaceNumber, size, weight, insuranceCost
		return string(s);
	}
};
