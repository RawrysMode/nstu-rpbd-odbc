...
Order::Order(int id, string companyName, int wagonNumber, string shippingDate, int shippingCost) {
    this->id = id;
...
Order Order::load(HDBC hDBC) {
    ...
    try {
        ...
        checkRetCodeException(SQLBindCol(hStmt, 2, SQL_C_LONG, &wagonNumber, 1, &len));
        checkRetCodeException(SQLBindCol(hStmt, 2, SQL_C_CHAR, &shippingDate, LEN, &len));
        checkRetCodeException(SQLBindCol(hStmt, 2, SQL_C_LONG, &shippingCost, 1, &len));
